...
import { Star, ArrowRight, Users, Award, MapPin, Loader2 } from 'lucide-react';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        const [chefsRes, recipesRes] = await Promise.all([
          supabase.from('chefs').select('*').limit(3),
          supabase.from('recipes').select('*, chefs(name)').limit(3)
        ]);

        if (chefsRes.data) {
          const mappedChefs = chefsRes.data.map(chef => ({
            ...chef,
            image: chef.image_url,
            reviews: chef.reviews_count,
            price: chef.hourly_price,
            distance: 'N/A', // Omitted from schema
            availability: 'Check Slots', // Omitted from schema
            dishes: 10, // Mock fallback
            maxGuests: 10,
          }));
          setFeaturedChefs(mappedChefs);
        }

        if (recipesRes.data) {
          const mappedRecipes = recipesRes.data.map(recipe => ({
            ...recipe,
